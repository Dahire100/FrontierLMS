...
import { ListChecks, Filter, Calendar, DollarSign, Receipt, TrendingUp, X } from "lucide-react"
...
export default function IncomeList() {
  ...
  const fetchHeads = async () => {
    ...
    } catch (e) { console.error("Failed to fetch heads", e) }
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIncomes(mappedArray.map((item: any) => ({
          ...
          date: item.incomeDate || item.date ? new Date(item.incomeDate || item.date).toLocaleDateString() : 'N/A',
          amount: item.amount,
  ...
  const clearFilters = () => {
    setFilters({ incomeHead: "all", dateFrom: "", search: "" })
    // Optionally fetch immediately or let user click search
    // fetchIncomes() // Let's not fetch immediately to follow pattern
  }
