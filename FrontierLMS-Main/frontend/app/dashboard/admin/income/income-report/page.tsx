...
import { BarChart2, FileText, Download, TrendingUp, Receipt, DollarSign, Loader2 } from "lucide-react"
...
export default function IncomeReport() {
    ...
    const fetchHeads = async () => {
        try {
            ...
            console.error("Failed to fetch income heads", e)
        }
    }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if ((data.report || []).length === 0) {
                    toast({ title: "Info", description: "No data found for the selected period." })
                } else {
                    toast({ title: "Success", description: "Report generated successfully" })
                }
            } else {
                toast({ title: "Error", description: "Failed to generate report from server.", variant: "destructive" })
            }
        } catch (error) {
            console.error('Error generating report:', error)
            toast({ title: "Error", description: "An error occurred while generating the report.", variant: "destructive" })
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleExportCSV = () => {
        ...
        const headers = ["Revenue Category", "Transaction Count", "Total Amount"]
        ...
        // Add total row
        const totalAmount = report.reduce((sum, r) => sum + r.amount, 0)
        const totalCount = report.reduce((sum, r) => sum + r.count, 0)
        rows.push(["TOTAL", totalCount.toString(), totalAmount.toFixed(2)])
                                        ...
                                        {report.map((row) => (
                                            <TableRow key={row.head}>
                                                ...
                                                <TableCell className="text-right text-indigo-600 font-semibold">
                                                    {stats.totalAmount > 0 ? ((row.amount / stats.totalAmount) * 100).toFixed(1) : 0}%
                                                </TableCell>
                                            </TableRow>
                                        ))}
