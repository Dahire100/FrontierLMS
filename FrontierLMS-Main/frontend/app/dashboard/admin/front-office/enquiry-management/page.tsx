...
import { ViewEnquiryModal } from "@/components/front-office/view-enquiry-modal"
...
export default function EnquiryManagementPage() {
  ...
  const fetchEnquiries = async () => {
    setLoading(true)
    try {
      const response = await apiFetch(API_ENDPOINTS.ENQUIRY);
      if (response.ok) {
  ...
  const handleEdit = async (id: string, data: any) => {
    try {
      const response = await apiFetch(`${API_ENDPOINTS.ENQUIRY}/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data)
      });
      if (response.ok) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await apiFetch(`${API_ENDPOINTS.ENQUIRY}/${deleteConfirm.id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
  ...
  const formFields: FormField[] = [
    ...
    { name: "source", label: "Lead Source (Origin)", type: "text", required: true, placeholder: "Ref, Web, Ad..." },
    {
      name: "status",
    ...
    <DashboardLayout title="Enquiry Command Center">
      <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
        ...
        <AdvancedTable
          ...
          searchFields={['phone', 'studentName', 'id']}
        ...
        <ViewEnquiryModal
          ...
          enquiry={enquiries.find(e => e.id === viewingId)}
        />
