...
export default function ParentFees() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dashRes = await fetch(`${API_URL}/api/parent/dashboard`, { headers })
        ...
        if (dashData.success) {
          ...
          const feePromises = kids.map(async (child: any) => {
            const feeRes = await fetch(`${API_URL}/api/parent/child/${child._id}/fees`, { headers })
  ...
  const handlePayNow = async (amount: number, feeTitle: string) => {
    try {
      ...
      const customerId = `PARENT_${Date.now()}` // Ideally use parent ID
