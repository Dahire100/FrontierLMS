...
import { Shield, Loader2, Eye, Printer, Download } from "lucide-react"
...
export default function CharacterCertificate() {
  ...
  const fetchSchoolSettings = async () => {
    try {
      ...
      const settings = data.data || data // Handle maybe wrapped response
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true)
      const token = localStorage.getItem("token")
      const response = await fetch(`${API_URL}/api/certificates?certificateType=character`, {
        headers: { "Authorization": `Bearer ${token}` }
      })
      const data = await response.json()
      if (data.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.studentId || !form.issuedBy) {
      toast.error("Student and issued by are required")
      return
    }
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/certificates`, {
        ...
        body: JSON.stringify({
          ...
          description: `Conduct: ${form.conduct}${form.remarks ? `\nRemarks: ${form.remarks}` : ''}`,
      ...
      if (data.success) {
        toast.success("Character certificate generated successfully!")
        setForm({
          studentId: "",
          conduct: "Excellent",
          remarks: "",
  ...
  const generatePDF = (cert: Certificate) => {
    ...
    doc.setFontSize(24)
    doc.setTextColor(0)
    doc.text("CHARACTER CERTIFICATE", 105, 90, { align: "center" })
    ...
    const bodyText = `This is to certify that Mr./Ms. ${studentName}, Roll No. ${rollNo}, is a student of this institution.`
    ...
    doc.text("To the best of my knowledge, he/she bears a good moral character.", 30, 185)
  ...
  const handleDownload = (cert: Certificate) => {
    ...
    doc.save(`${cert.certificateNumber}_Character.pdf`)
  }
      ...
      <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-red-500">Conduct *</Label>
                  <Select value={form.conduct} onValueChange={(value) => setForm({ ...form, conduct: value })}>
                    <SelectTrigger className="bg-white border-gray-200">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="Additional remarks about student's character and behavior..."
                  className="bg-white border-gray-200"
                  rows={3}
                  value={form.remarks}
        ...
        <Card>
          <CardHeader className="bg-gray-50 border-b border-gray-100">
            <CardTitle>Issued Character Certificates Log</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
