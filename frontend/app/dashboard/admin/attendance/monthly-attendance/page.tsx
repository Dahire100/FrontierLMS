...
export default function MonthlyAttendance() {
    ...
    const generateReport = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/api/attendance/monthly-report?${params.toString()}`, {
                headers: { "Authorization": `Bearer ${token}` }
            })
            const result = await res.json()
            const list = Array.isArray(result)
            ...
            if (Array.isArray(list)) {
                ...
                setReportData([])
                toast({ title: "No Data", description: result?.error || "Failed to generate report", variant: "destructive" })
            }
        } catch (err) {
                        ...
                        <div className="p-6">
                            ...
                            <div className="mt-1 text-sm text-gray-600">
                                {hasGenerated
                                    ...
                                    : "Select a class, month and year above, then click Generate."}
                            </div>
                        </div>
                    </Card>
                )}
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-pink-50 hover:bg-pink-50">
                                            ...
                                            <TableHead className="font-bold text-gray-700 uppercase text-center">Total Days</TableHead>
                                            ...
                                            <TableHead className="font-bold text-gray-700 uppercase text-center">%</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {reportData
                                            ...
                                            .map((row) => (
                                                <TableRow key={row.studentId}>
                                                    ...
                                                    <TableCell className="text-center text-green-700 font-bold">{row.presentDays}</TableCell>
                                                    <TableCell className="text-center text-red-600 font-bold">{row.absentDays}</TableCell>
                                                    <TableCell className="text-center">
                                                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${parseFloat(row.attendancePercentage) >= 75 ? 'bg-green-100 text-green-800' :
                                                                parseFloat(row.attendancePercentage) >= 50 ? 'bg-yellow-100 text-yellow-800' :
